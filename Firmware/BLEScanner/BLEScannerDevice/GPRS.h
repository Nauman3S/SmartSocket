...
void setupGPRS()
{
  ...
  modem.gprsConnect(apnV, gprsUser, gprsPass);
#endif
  ...
  SerialMon.print(apnV);

  // if (!modem.gprsConnect(apnV, gprsUser, gprsPass))
  if (!modem.gprsConnect(apnV, gprsUser, gprsPass))
  {
    SerialMon.println(" fail");
    delay(10000);
...
void loopGPRS(String scanResults)
{
  ...
  if (!mqtt.connected())
  {
    ...
    mqttPublishData("SmartEFM/ble/data", scanResults);
